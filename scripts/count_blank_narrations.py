#!/usr/bin/env python3
"""Count scenes with a blank narration in a scripts JSON file."""
...
def count_blank_narrations(payload: Any) -> int:
    ...
    for entry in entries:
        ...
        for scene in scenes:
            if not isinstance(scene, dict):
                continue
            if _is_blank(scene.get("narration")):
                total += 1
    return total
...
def main(argv: list[str] | None = None) -> int:
    ...
    total = count_blank_narrations(load_scripts(args.scripts_file))
    json.dump({"blank_narration_count": total}, sys.stdout)
    sys.stdout.write("\n")
    return 0
