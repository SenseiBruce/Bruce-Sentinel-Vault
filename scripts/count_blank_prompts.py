#!/usr/bin/env python3
"""Count scenes with a blank image_prompt in a scripts JSON file."""
...
def count_blank_prompts(payload: Any) -> int:
    ...
    for entry in entries:
        ...
        for scene in scenes:
            if not isinstance(scene, dict):
                continue
            if _is_blank(scene.get("image_prompt")):
                total += 1
    return total
...
def main(argv: list[str] | None = None) -> int:
    ...
    total = count_blank_prompts(load_scripts(args.scripts_file))
    json.dump({"blank_prompt_count": total}, sys.stdout)
    sys.stdout.write("\n")
    return 0
